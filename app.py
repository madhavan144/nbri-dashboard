...
# 2. Remove default Streamlit whitespace/padding for a clean full-screen look
st.markdown("""
    <style>
        .block-container {
...
<head>
    ...
    <style>
        body {
            ...
            background: radial-gradient(circle at 20% 20%, #171b38 0%, #090b15 60%, #05060d 100%);
...
<body class="min-h-screen pb-10">
    ...
    <!-- TOP NAVIGATION BAR -->
    <header class="sticky top-0 z-50 glass-panel border-b border-slate-800 px-6 py-3.5 mb-6">
        <div class="max-w-[1700px] mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            
            <div class="flex items-center gap-4">
                <div id="logo-container" class="w-11 h-11 rounded-xl bg-gradient-to-tr from-cyan-500 via-blue-600 to-purple-600 flex items-center justify-center text-white shadow-lg shadow-cyan-500/20">
                    <i class="fa-solid fa-house-shield text-xl"></i>
                </div>
                <div>
            ...
            <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                <div class="relative flex-1 md:flex-initial">
                    ...
                    <select id="filter-region" class="w-full bg-slate-900/80 text-xs text-slate-200 pl-8 pr-8 py-2 rounded-xl border border-slate-700/60 focus:border-cyan-500 focus:outline-none appearance-none cursor-pointer">
                ...
                <div class="relative flex-1 md:flex-initial">
                    ...
                    <select id="filter-district" class="w-full bg-slate-900/80 text-xs text-slate-200 pl-8 pr-8 py-2 rounded-xl border border-slate-700/60 focus:border-cyan-500 focus:outline-none appearance-none cursor-pointer">
                ...
                <div class="relative flex-1 md:flex-initial">
                    ...
                    <input type="text" id="search-input" placeholder="Search estate or division..." class="w-full bg-slate-900/80 text-xs text-slate-200 pl-8 pr-4 py-2 rounded-xl border border-slate-700/60 focus:border-cyan-500 focus:outline-none placeholder-slate-500">
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-8 flex flex-col gap-6">
                ...
                <div class="glass-panel rounded-2xl p-5 relative flex flex-col">
                    ...
                    <div class="w-full h-[430px] rounded-xl overflow-hidden relative border border-slate-800" id="map-container">
                ...
                <div class="glass-panel rounded-2xl p-5 overflow-hidden">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-xs text-slate-300">
                            <thead class="bg-slate-900/90 text-slate-400 font-semibold uppercase border-b border-slate-800">
                                <tr>
                                    ...
                                    <th class="py-3 px-3 text-center">BOD Completed</th>
                                </tr>
                            </thead>
            ...
            <div class="lg:col-span-4 flex flex-col gap-6">
                ...
                <!-- AI ASSISTANT -->
                <div class="glass-panel-glow rounded-2xl p-5 flex flex-col flex-1">
                    ...
                    <div id="ai-chat-box" class="flex-1 min-h-[160px] max-h-[220px] overflow-y-auto space-y-3 p-3 bg-slate-950/50 rounded-xl border border-slate-800/80 text-xs mb-3">
                        <div class="flex gap-2.5">
                            ...
                            <div class="bg-slate-900/90 text-slate-200 p-2.5 rounded-xl border border-slate-800">
                                Ayubowan! I am your NBRO Resettlement Data Assistant. Ask me about site stats or reports.
                            </div>
                        </div>
                    </div>
                    <div class="relative">
                        <input type="text" id="ai-input" onkeypress="handleAIPress(event)" placeholder="Ask LIRIS AI..." class="w-full bg-slate-900 text-xs text-slate-200 pl-3 pr-10 py-2.5 rounded-xl border border-slate-700 focus:border-purple-500 focus:outline-none">
    ...
    <script>
        ...
        function processCSVRows(rows) {
            rawData = rows.map((r, idx) => {
                ...
                const units = parseInt(r['Units (2020 List)'] || r['Units'] || 0) || 0;
                const reportIssued = (r['NBRI 1st Report Issued'] || 'Yes').trim();
                const bodCompleted = (r['BOD Completed'] || 'None').trim();

                let baseCoords = districtCoordinates[district] || { lat: 6.68, lng: 80.39 };
                let lat = parseFloat(r['Latitude']) || (baseCoords.lat + (Math.random() - 0.5) * 0.15);
                let lng = parseFloat(r['Longitude']) || (baseCoords.lng + (Math.random() - 0.5) * 0.15);

                return { sno: idx + 1, region, district, estate, division, units, reportIssued, bodCompleted, lat, lng };
            });
        ...
        function updateMapMarkers() {
            ...
            filteredData.forEach(site => {
                ...
                marker.bindPopup(`<b>${site.estate}</b><br>District: ${site.district}<br>Units: ${site.units}`);
        ...
        function initCharts() {
            ...
            barChartInstance = new Chart(ctxBar, {
                type: 'bar',
                data: { labels: [], datasets: [{ label: 'Units', data: [], backgroundColor: '#3b82f6', borderRadius: 6 }] },
            ...
            doughnutChartInstance = new Chart(ctxDoughnut, {
                type: 'doughnut',
                data: { labels: ['Issued', 'Pending'], datasets: [{ data: [0, 0], backgroundColor: ['#06b6d4', '#f59e0b'] }] },
        ...
        function updateCharts() {
            ...
            filteredData.forEach(item => {
                districtMap[item.district] = (districtMap[item.district] || 0) + item.units;
            });

            barChartInstance.data.labels = Object.keys(districtMap);
            barChartInstance.data.datasets[0].data = Object.values(districtMap);
        ...
        function renderTable() {
            ...
            filteredData.forEach(row => {
                ...
                tr.className = 'hover:bg-slate-800/40 border-b border-slate-800/40';
                tr.innerHTML = `
                    <td class="py-3 px-3 font-mono">${row.sno}</td>
                    <td class="py-3 px-3 font-semibold">${row.region}</td>
                    <td class="py-3 px-3">${row.district}</td>
                    <td class="py-3 px-3 font-bold text-cyan-300">${row.estate}</td>
                    <td class="py-3 px-3">${row.division}</td>
                    <td class="py-3 px-3 text-right font-bold">${row.units}</td>
        ...
        function sendAIMessage() {
            ...
            setTimeout(() => {
                chatBox.innerHTML += `<div class="flex gap-2 mb-2"><div class="bg-slate-900 p-2 rounded-xl text-slate-200">Showing stats for active filters: <strong>${filteredData.length} sites</strong> matching.</div></div>`;
...
# 4. Render HTML inside Streamlit with height
